...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $uploadOk = 1;
    $imageFileType = strtolower(pathinfo($target_file_poster, 
    PATHINFO_EXTENSION));
    ...
    echo "Maaf, file poster tidak dapat diupload.";
        ...
        if (move_uploaded_file($_FILES["poster"]["tmp_name"], 
        $target_file_poster)) {
        ...
        $uploadOkBanner = 1;
        $imageFileTypeBanner = strtolower(pathinfo($target_file_banner, 
        PATHINFO_EXTENSION));
        // Cek apakah file gambar adalah gambar sebenarnya
        ...
        if (!in_array($imageFileTypeBanner, ['jpg', 'png', 'jpeg', 
        'gif'])) {
        echo "Maaf, hanya file JPG, JPEG, PNG & GIF yang diperbolehkan 
        untuk banner.";
        $uploadOkBanner = 0;
        ...
        $uploadOkTrailer = 1;
        $videoFileType = strtolower(pathinfo($target_file_trailer, 
        PATHINFO_EXTENSION));
...
if (move_uploaded_file($_FILES["trailer"]["tmp_name"], 
$target_file_trailer)) {
// Menyiapkan dan mengeksekusi query
$stmt = $conn->prepare("INSERT INTO film_shella (poster, 
trailer, banner, nama_film, judul, total_menit, usia, genre, 
dimensi,Producer,Director,Writer,Cast,Distributor,harga) 
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?,?,?)");
$stmt->bind_param("sssssssssssssss", 
$target_file_poster, $target_file_trailer, $target_file_banner, 
$nama_film, $judul, $menit, 
$usia, $genre, 
$dimensi,$producer,$director,$writer,$cast,$distributor,$harga);
